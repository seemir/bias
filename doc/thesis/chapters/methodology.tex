...
\noindent The random forest algorithm \citep{ho1995random} is a decision tree based ensemble learning classifier that is much used for both classification and regression tasks. The random forest algorithm used a multitude of decision trees to classify the outcome/class of a classification problem. There is also an important part about the decision trees that the algorithm generates, and that is that they are decorrelated. The decision trees are build using the bootstrap re-sampling algorithm, and each time a split in the decision tree is considered, a random sample of $m$ predictors are chosen from the full sample of 
\begin{algorithm}[H]{
\SetAlgoLined
\textit{initialization}\;
    \hspace{0.5cm} $\mathbf{X}$: training data\\
...
\noindent The final classification algorithm is the least absolute shrinkage and selection operator (LASSO) algorithm \citep{tibshirani1996regression}. This algorithm can be used for classification, but is often consider to be a regression methods that performs both variable selection and regularization. The LASSO algorithm seeks to solve the following optimization problem:
\begin{align}
    \min_{\beta}\left \{\sum_{i=1}^n\left(y_i - \beta_0 - \sum_{j=1}^p\beta_jx_{ij} \right)^2 \right\} \hspace*{0.5cm} \text{subject to} \hspace*{0.5cm} \sum_{j=1}^P|\beta_j| \leq s
\end{align}
\noindent This optimization problem can also be written with in the Lagrangian form:
\begin{align}
    \beta^{\text{lasso}} = \operatorname {arg\,min}_{\beta}\left \{\frac{1}{2}\sum_{i=1}^N\left(y_i - \beta_0 - \sum_{j=1}^Px_{ij}\beta_j \right)^2 + \lambda\sum_{j=1}^p|\beta_j| \right\}    
\end{align}
\noindent The LASSO has a tuning parameter $\lambda$ and the term $\lambda \sum_{j=1}^p|\beta_j|$ is a shrinkage penalty term that has the effect of shrinking the estimates of $\beta_j$ towards zero. The tuning parameter $\lambda$ controls the relative impact of the two terms in the Lagrangian form defined above, i.e. when $\lambda = 0$ the $\beta^{\text{lasso}}$ is defined by a standard least squares estimate, see \citep{friedman2009elements} and \citep{james2013introduction}.\\
\indent The LASSO has many advantages, the first being that it can provide good prediction accuracy in classification settings. This is especially the case when the number of observations are much smaller than the number of features, i.e. $n \ll p$. The tuning parameter is also a defined parameter that can be used to represent the trade-off between bias and variance, i.e. when the tuning parameter $\lambda$ increases the variance decreases. The LASSO also increases the model interpretability as it only includes variable selection, i.e. it eliminates the irrelevant variables that are not relevant for the response variable and by doing this overfitting is also reduced. The implementation of the LASSO algoroithm for the classification is done using the \texttt{glmnet()} function from the \texttt{glmnet} package in \texttt{r} \citep{glmnet}.

\section{Cross-Validation}
\label{sec:validation}

\noindent When talking about evaluating the performance of a given classification algorithm one typically mentions the test error rate, i.e. the average prediction error that results from using a statistical learning algorithm. The most common way of estimating the average prediction error is through the way of cross-validation (CV). This is a direct method of estimating the expected extra-sample error $Err = E\left[L\left(Y, \hat{f}\left(X \right) \right), \right]$, i.e. the average generalization error when the method $\hat{f}\left(X \right)$ is applied to an independent test sample from the joint distribution of $X$ and $Y$, see \citep{friedman2009elements} and \citep{james2013introduction}. In this section, we present the two most common ways of estimating the test error rate of a given statistical learning algorithm.

\subsection{K-Fold}
\label{subsec:k_fold}

\noindent In the $K$-fold cross-validation method \citep{geisser1975predictive} one typically splits the data into $K$ roughly equal-sized parts (also called folds) and for a $k$th part, we fit the model on the remaining $K-1$ parts of the data. This is done for $k = 1, \hdots, K$ and after this is done we are left with $K$ estimates of the prediction error. This prediction error is typically defined by the mean square error (MSE). After producing the mean square error for the $k = 1, \hdots, K$ folds, we average the MSEs to produce the $k$-fold cross-validation estimate. The formula is given by the following:
\begin{align}
    CV_k = \frac{1}{nk}\sum_{i=1}^k\sum_{j=1}^n\left(Y_{ij} - \hat{Y}_{ij} \right)^2
\end{align}
\noindent The $K$-fold cross-validation estimate is one of many criteria used to evaluate the performance of various classifiers. One clear advantage of using the $K$-fold cross-validation estimate is computational, i.e. the runtime properties of the $K$-fold cross-validation algorithm are good as one limits the number of splits of the data to $K$-folds. This can also lower the variance of the prediction error since there is a high chance that all the K-folds are more different than if one was to chose $K = N$ (also called leave-one-out cross validation), see \citep{friedman2009elements}. In the setting of this thesis, we will evaluate the classification algorithm mentioned earlier using the most common CV algorithm (K-fold and leave-one-out) simply because we want to examine how the test error varies given the choice of CV method. Furthermore, the psedo-code of the $K$-fold algorithm is given below. The implementation of the $K$-fold cross-validation method is typically included in each of the implementations of the classification algorithms mentioned in the previous sections.
\begin{algorithm}[H]{
\SetAlgoLined
\textit{initialization}\;
    \hspace{0.5cm} $\mathbf{X}$: training data\\
...
\noindent The next method examined is the leave-one-out cross-validation (LOOCV) method \citep{stone1974cross}. Contrary to the $K$-fold, that splits the data into $K$ parts, LOOCV splits the data into $n$ parts where a single observation $\left(x_1, y_1 \right)$ is used for validation and the rest of the observations $\left \{(x_2, y_2), \hdots, (x_n, y_n) \right \}$ are used as the training set. The methods fits the statistical learning algorithm to be evaluated on the remaining $n - 1$ training observations, and a prediction $\Hat{y}_i$ is made on the excluded observation. For this excluded observation, the algorithm uses the predicted value $\Hat{Y}_i$ to estimate the mean square error (MSE), and this procedure is continued for the remaining $n-1$ training observations. With the $n$ MSE, the LOOCV estimate becomes, see \citep{friedman2009elements} and \citep{james2013introduction}:
\begin{align}
    CV_{loocv} = \frac{1}{n^2}\sum_{i=1}^n\sum_{j=1}^n\left(Y_{ij} - \Hat{Y}_{ij} \right)^2
\end{align}
\noindent Of the advantages that can be attributed to the LOOCV algorithm, one typically mention that it has less bias than other validation algorithms, such as the validation set approach. The results that are produced by the algorithm are also "consistent", i.e. they will be more similar compared to other CV algorithms. This is due the fact that there is no randomness in the training/validation set splits. This does, however, not mean that the LOOCV algorithm has no variability, however compared to the validation set approach the variability is less. The disadvantages are exactly the fact that the LOOCV can have variability that is less that some algorithms, but at the same time more than others. An example can be if one compares LOOCV with $K$-fold, as only one observation is used in the validation set at any given time, the variability can be larger then that of the $K$-fold algorithm. In addition, the LOOCV algorithm can be computationally more expensive (both in time and space) compared to other CV algorithms. This is especially the case for large data sets, as $n$ is large the LOOCV algorithm requires fitting the model $n$ times. In addition some statistical learning algorithms may have intensive or "heavy" fitting processes that adds to the computational cost. The psedo code for LOOCV is similar to that of $K$-fold, with the exception of two aspects: (i) the user does not need to specify the number of folds to be used and (ii) as a results of (i) the algorithm does not iterate over the K-folds when fitting the learning algorithm, but rather does so over all $n$ splits of the data set. As a closing remark, both the $K$-fold and LOOCV algorithms are much used in evaluating the prediction error of classification algorithms. Accordingly, the variability between these two algorithms is one we wish to examine to the full extent in the context of classifying the clinical outcomes of HF patients. 
